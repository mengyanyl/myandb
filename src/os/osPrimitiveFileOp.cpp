...
osPrimitiveFileOp::osPrimitiveFileOp()
{
   _fileHandle = OSS_INVALID_HANDLE_FD_VALUE ;
   _bIsStdout  = false ;
}

bool osPrimitiveFileOp::isValid ()
{
   return ( OSS_INVALID_HANDLE_FD_VALUE != _fileHandle ) ;
}

void osPrimitiveFileOp::Close()
{
...
int osPrimitiveFileOp::Open( const char * pFilePath, unsigned int options )
{
   int rc = 0 ;
   int mode = O_RDWR ;
...
void osPrimitiveFileOp::openStdout()
{
   setFileHandle(STDOUT_FILENO) ;
   _bIsStdout = true ;
}

offsetType osPrimitiveFileOp::getCurrentOffset () const
{
  return oss_lseek( _fileHandle, 0, SEEK_CUR ) ;
}

void osPrimitiveFileOp::seekToEnd( void )
{
   oss_lseek( _fileHandle, 0, SEEK_END ) ;
}

void osPrimitiveFileOp::seekToOffset( offsetType offset )
{
...
int osPrimitiveFileOp::Read ( const size_t size,
                               void * const pBuffer,
...
int osPrimitiveFileOp::Write( const void * pBuffer, size_t size )
{
   int rc = 0 ;
...
int osPrimitiveFileOp::fWrite( const char * format, ... )
{
   int rc = 0 ;
   va_list ap ;
...
void osPrimitiveFileOp::setFileHandle( handleType handle )
{
   _fileHandle = handle ;
}

int osPrimitiveFileOp::getSize( offsetType * const pFileSize )
{
   int             rc        = 0 ;
