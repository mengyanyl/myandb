...
void Myandb::start(void)
{
   std::cout << "Welcome to MyanDB Shell!" << std::endl;
   std::cout << "myandb help for help, Ctrl+c or quit to exit" << std::endl;
   while(0==gQuit)
   {
      prompt();
   }
}
void Myandb::prompt(void)
{
   int ret = EDB_OK;
   ret = readInput("mydb", 0);
...
int Myandb::readInput(const char *pPrompt, int numIndent)
{
...
char *Myandb::readLine(char *p, int length)
{
   int len = 0;
   int ch;
...
void Myandb::split(const std::string &text, char delim, std::vector<std::string> &result)
{
